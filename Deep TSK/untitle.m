        ...
        switch dp 
            ...
            %                 H(k, i) = min(V(k, (1:d), i));
                    ...
                    %% Learn the parameters in the then-parts in fuzzy rules of the current layer dp using GD
                        ...
                        [accuracy, TP, TN, FP, FN, rulesNumber] = Prediction(X, Y_actual, 1, K);

                        table(accuracy, TP, TN, FP, FN, rulesNumber)
                        y_dp = y1;
%                 end
                disp('--------------------------------------')
            case 2
...
%                         A1 = zeros(N, 1);
                        ...
                        for i = 1 : N
%                             size(q_g(1, 1:K(dp - 1), 1))
%                             size(H(1:K(dp - 1), i))
%                             A1(i, 1) = sum(q_g(1, 1:K(dp - 1), 1) * H(1:K(dp - 1), i)); % in q_g the first coefficient is rL_k
                            A2(i, 1) = sum(q_g(2, 1:K(dp), 1) * mu_y1(:, i) * H(:, i)); % first coefficients in second layer are r2_k
                            B2_1(i, 1) = sum(q_g(2, 1:K(dp), 2) * mu_y1(:, i) * H(:, i));
                        end
                        y2 = A2 + A1 .* B2_1;
%                         save HID-TSK-FC.mat G sigma_G centers theta omega q_g
%                         [accuracy, TP, TN, FP, FN, rulesNumber] ...
%                             = Prediction(X, Y_actual, 2, K);
                        E_prev = E_current;
